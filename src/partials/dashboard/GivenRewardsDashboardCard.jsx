...
function GivenRewardsDashboardCard() {
  ...
  const chartData = {
    labels: [texts.coffee, texts.cake, texts.meal, texts.childToy, texts.discount],
    datasets: [
      {
        label: texts.givenRewards, 
        data: [
          536, 719, 397, 230, 120
        ],
        backgroundColor: [
  ...
  return (
    <div className="flex flex-col col-span-full sm:col-span-4 xl:col-span-4 bg-white dark:bg-gray-800 shadow-xs rounded-xl">
      <header className="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60">
        <h2 className="font-semibold text-gray-800 dark:text-gray-100">{texts.givenRewardsWeekly}</h2>
      </header>
      {/* Chart built with Chart.js 3 */}
      {/* Change the height attribute to adjust the chart height */}
      {/* Lang force la recreation du doghnut chart */}
      <DoughnutChart key={lang} data={chartData} width={389} height={260} />
    </div>
  );
}
